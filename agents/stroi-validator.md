...
Detect the project's package manager / scripts (or read the commands from the in-scope `CLAUDE.md` map block's `## Commands`), then run, in order:
...
Also apply any `verify`-relevant guidance from the in-scope `CLAUDE.md`'s `## Relevant Skills`.
